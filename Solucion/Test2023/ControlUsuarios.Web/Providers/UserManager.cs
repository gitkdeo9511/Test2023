...
namespace ControlUsuarios.Web.Providers
{
    public interface IUserManager
    {
        Task SignIn(HttpContext httpContext, JWT user, bool isPersistent = false);
        Task SignOut(HttpContext httpContext);
    }

    public class UserManager : IUserManager
    {
        public async Task SignIn(HttpContext httpContext, JWT user, bool isPersistent = false)
        {
        ...
        private List<Claim> GetUserClaims(JWT user)
        {
            List<Claim> claims = new List<Claim>();
            claims.Add(new Claim(ClaimTypes.NameIdentifier, user.UserName.ToString()));
            claims.Add(new Claim(ClaimTypes.Name, user.UserName));
            claims.Add(new Claim(ClaimTypes.Role, user.RolPermiso.Rol.Name));
            return claims;
        }
    }
}